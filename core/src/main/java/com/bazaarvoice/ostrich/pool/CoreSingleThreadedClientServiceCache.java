 ...
 */
class CoreSingleThreadedClientServiceCache<S> implements ServiceCache<S> {
     ...
     */
    @VisibleForTesting CoreSingleThreadedClientServiceCache(ServiceCachingPolicy policy, ServiceFactory<S> serviceFactory,
                                                            ScheduledExecutorService executor, Metrics metrics) {
        checkNotNull(policy);
        ...
        _metrics.registerGauge(serviceName, "cache-hit-ratio", new RatioGauge() {
        ...
        _metrics.registerGauge(serviceName, "cache-miss-ratio", new RatioGauge() {
            @Override protected double getNumerator() { return _missCount.get(); }

            @Override protected double getDenominator() { return _requestCount.get(); }
        });

        _metrics.registerGauge(serviceName, "load-success-ratio", new RatioGauge() {
        ...
        _metrics.registerGauge(serviceName, "load-failure-ratio", new RatioGauge() {
    ...
    @VisibleForTesting GenericKeyedObjectPool<ServiceEndPoint, S> getPool() {
        return _pool;
    }
