...
import com.bazaarvoice.ostrich.spi.Counter;
import com.bazaarvoice.ostrich.spi.ScopedMetrics;
import com.bazaarvoice.ostrich.spi.Timer;
 ...
 */
class CoreMultiThreadedClientServiceCache<S> implements ServiceCache<S> {
     ...
     */
    CoreMultiThreadedClientServiceCache(MultiThreadedServiceFactory<S> serviceFactory, ScopedMetrics metrics) {
        this(serviceFactory, buildDefaultExecutor(), DEFAULT_EVICTION_DELAY_SECONDS, DEFAULT_CLEANUP_DELAY_SECONDS, metrics);
    }
     ...
     */
    @VisibleForTesting CoreMultiThreadedClientServiceCache(MultiThreadedServiceFactory<S> serviceFactory, ScheduledExecutorService executor,
                                                           int evictionDelayInSeconds, int cleanUpDelayInSeconds, ScopedMetrics metrics) {
        ...
        _cleanupFuture = _cleanupExecutor.scheduleAtFixedRate(
            new Runnable() {
                @Override
                public void run() {
                    List<HeavyServiceHandle<S>> handlesToDelete = new LinkedList<HeavyServiceHandle<S>>();
                    ...
                    synchronized (CoreMultiThreadedClientServiceCache.this) {
    ...
    private ServiceHandle<S> doRegister(ServiceEndPoint endPoint) {
        ...
        synchronized (this) {
            ...
            if (existingServiceHandle == null || existingServiceHandle.hasBeenFlaggedForEviction() || existingServiceHandle.isOld()) {
                ...
                HeavyServiceHandle<S> newServiceHandle = new HeavyServiceHandle<S>(service, endPoint);
    ...
    public synchronized void evict(ServiceEndPoint endPoint) {
        checkNotNull(endPoint);
        TimerContext context = _evictionTimer.time();
