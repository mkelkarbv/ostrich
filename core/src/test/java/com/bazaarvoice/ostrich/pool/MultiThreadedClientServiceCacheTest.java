...
public class MultiThreadedClientServiceCacheTest {
    ...
    public void testCheckInNullServiceInstance()
            throws Exception {
        ServiceHandle<Service> handle = new ServiceHandle<Service>(null, _endPoint);
        newCache().checkIn(handle);
    }
    ...
    private MultiThreadedClientServiceCache<Service> newCache(int ttl) {
        MultiThreadedClientServiceCache<Service> cache = new MultiThreadedClientServiceCache<Service>(_factory, ServiceCacheBuilder.buildDefaultExecutor(), ttl, ttl);
        _caches.add(cache);
        return cache;
    }
